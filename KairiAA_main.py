...
def main():
  ...
  while(flg_running):
    ...
    for e in p.event.get():
      if e.type == p.QUIT:
        ...
        if e.key == p.K_s:
          ...
          if not flg_gameover:
            if(player_one_human and gs.sente_to_move ) or (player_two_human and not gs.sente_to_move ):
              # クリックした場所の把握
    ...
    if not flg_gameover:
      if not((player_one_human and gs.sente_to_move ) or (player_two_human and not gs.sente_to_move )):
        if(len(valid_moves) == 0):
          gs.mate = True
        else:
          score, AI_move = KairiAa_AI.get_Minimax_move(gs, valid_moves, KairiAa_AI.DEPTH, gs.sente_to_move)
          print(f"Score AI calculated: {score}")
          gs.make_move(AI_move[0], AI_move[1])
          valid_moves = gs.get_valid_moves()

    draw_game_status(screen, gs, valid_moves, square_clicked)

    if(gs.counter >= 100000):
      flg_gameover = True
      print("AAA")
      print(gs.moveLog)
